...
def build_source_display_text(source, page_number=None):
    """
    参照元の表示文言を作成（PDFの場合のみページ番号を追記）
    ...
    # ページ番号はPDFファイルのときのみ表示
    if page_number is None or not source_text.lower().endswith(".pdf"):
        return source_text

    # PyMuPDFLoaderのページ番号（0始まり想定）を画面表示用に1始まりへ変換
    try:
        display_page_number = int(page_number) + 1
    except (TypeError, ValueError):
        return source_text

    return f"{source_text} (ページNo.{display_page_number})"
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"]:
                            main_file_info = build_source_display_text(
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    sub_file_info = build_source_display_text(
                                        sub_choice['source'],
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata:
            ...
            main_file_info = build_source_display_text(main_file_path, main_page_number)
            st.success(main_file_info, icon=icon)
        else:
        ...
        if sub_choices:
            ...
            for sub_choice in sub_choices:
                ...
                if "page_number" in sub_choice:
                    ...
                    sub_file_info = build_source_display_text(sub_choice['source'], sub_choice['page_number'])
                    st.info(sub_file_info, icon=icon)
                else:
                    # 「サブドキュメントのファイルパス」を表示
                    st.info(f"{sub_choice['source']}", icon=icon)
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            if "page" in document.metadata:
                ...
                # 「ファイルパス」と「ページ番号」
                file_info = build_source_display_text(file_path, page_number)
            else:
                # 「ファイルパス」のみ
